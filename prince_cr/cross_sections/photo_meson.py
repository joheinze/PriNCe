...
class SophiaSuperposition(CrossSectionBase):
    ...
    def incl(self, mother, daughter):
        ...
        if daughter <= 101:
            ...
            return self.egrid[[0, -1]], np.array([0.0, 0.0])
        ...
        if daughter in [mother - 101]:
            ...
            raise Exception(
                "Channel {:} to {:} not allowed in this superposition model".format(
                    mother, daughter
                )
            )
...
class EmpiricalModel(SophiaSuperposition):
    ...
    def A_eff(self, A, x):
        ...
        def sigm(x, shift=0.0, gap=1, speed=1, base=0.0, rising=False):
            ...
            base: minimal value
            """
            sigmoid = 1.0 / (1 + np.exp(-speed * (x - shift)))

            if rising:
                return gap * sigmoid + base
            else:
                return gap * (1.0 - sigmoid) + base
        ...
        for xi in x:
            if xi < 7:
                y.append(sigm(xi, 0.7, 0.099, 0.7, 0.942))
            elif xi < 300:
                y.append(np.interp(xi, nsc[0], nsc[1]))
            else:
                y.append(sigm(xi, 430, 0.2, 0.01, 0.66))
    ...
    def _fill_multiplicity(self, *args, **kwargs):
        ...
        for mom in nuclides:
            ...
            if (
                (mom < 101)
                or (A > config.max_mass)
                or isinstance(mom, str)
                or (spec_data[mom]["lifetime"] < config.tau_dec_threshold)
            ):
                continue
    ...
    def nonel(self, mother):
        ...
        e_scale = 0.3  # prefixed based on data
    ...
    def incl_diff(self, mother, daughter):
        ...
        if (mother, daughter) in self.multiplicity:
            ...
            cs_diff[-1, :] += self.multiplicity[mother, daughter] * cs_nonel / xw
        elif (mother > 101) and (
            daughter in [2, 3, 4]
            ...
            def superposition_incl(mother, daughter):
                ...
                _, Z, N = get_AZN(mother)
                cs_diff = (
                    self.redist_proton[daughter].T * Z * self.cs_proton_grid
                    + self.redist_neutron[daughter].T * N * self.cs_neutron_grid
                )
                cs_incl = trapz(
                    cs_diff, x=self.xcenters, dx=bin_widths(self.xbins), axis=0
                )
            ...
            cs_diff *= float(Am) ** (-1 / 3.0)  # ... rescaling SpM to A^2/3

            cs_incl_pi0_sophia = superposition_incl(mother, daughter) * float(Am) ** (
                -1 / 3.0
            )
            cs_incl_pi0_data = 1e-30 * self.pion_spl(egrid * 1e3) * Am ** (2.0 / 3)
            ...
            cs_diff_renormed = self.fade(
                cs_diff, cs_diff_renormed, range(32)
            ...
            alpha_plus = np.where(
                egrid <= 1.0, 2.0 / 3, 1 - np.exp(-4.0 / 7 * (egrid - 1) ** 0.5) / 3
            )
            cs_diff *= Am ** (alpha_plus - 2.0 / 3)
