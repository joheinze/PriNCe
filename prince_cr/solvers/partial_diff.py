...
class SemiLagrangianSolver(object):
    ...
    def interpolate_linear_weights(self, conloss, state):
        ...
        weight0 = (self.grid_log[i0] - newgrid_log[i1]) / (
            newgrid_log[i0] - newgrid_log[i1]
        )
        weight1 = (self.grid_log[i0] - newgrid_log[i0]) / (
            newgrid_log[i1] - newgrid_log[i0]
        )
    ...
    def interpolate_quadratic_weights(self, conloss, state):
        ...
        weight_b0 = (
            (self.grid_log[i1] - newgrid_log[i1])
            / (newgrid_log[i0] - newgrid_log[i1])
            * (self.grid_log[i1] - newgrid_log[i2])
            / (newgrid_log[i0] - newgrid_log[i2])
        )
        weight_b1 = (
            (self.grid_log[i1] - newgrid_log[i0])
            / (newgrid_log[i1] - newgrid_log[i0])
            * (self.grid_log[i1] - newgrid_log[i2])
        ...
        weight_f1 = (
            (self.grid_log[i1] - newgrid_log[i2])
            / (newgrid_log[i1] - newgrid_log[i2])
            * (self.grid_log[i1] - newgrid_log[i3])
    ...
    def interpolate_cubic_weights(self, conloss, state):
        ...
        state = np.zeros_like(self.grid_log)
        state[i1] = np.exp(
            weight0 * newstate_log[i0]
            + weight1 * newstate_log[i1]
            + weight2 * newstate_log[i2]
            + weight3 * newstate_log[i3]
        )
        return state
    ...
    def interpolate_4thorder_weights(self, conloss, state):
        ...
        weight_b0 = (
            ...
            * (self.grid_log[i2] - newgrid_log[i4])
            / (newgrid_log[i0] - newgrid_log[i4])
        )
        weight_b1 = (
            (self.grid_log[i2] - newgrid_log[i0])
            / (newgrid_log[i1] - newgrid_log[i0])
            * (self.grid_log[i2] - newgrid_log[i2])
            / (newgrid_log[i1] - newgrid_log[i2])
            * (self.grid_log[i2] - newgrid_log[i3])
    ...
    def interpolate_5thorder_weights(self, conloss, state):
        ...
        state[i2] = np.exp(
            ...
            + weight2 * newstate_log[i2]
            + weight3 * newstate_log[i3]
            + weight4 * newstate_log[i4]
            + weight5 * newstate_log[i5]
        )
        return state
