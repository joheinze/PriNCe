...
class CrossSectionCombined(CrossSection):
    ...
    def __init__(self, models, e_trans):
        ...
        for key in nonel1:
            if key in nonel2:
                self._nonel[key] = self._get_combined_cs(
                    nonel1[key], nonel2[key], e_trans)
            else:
        ...
        for key in incl1:
            if key in incl2:
                self._incl[key] = self._get_combined_cs(
                    incl1[key], incl2[key], e_trans)
            else:
...
class CrossSectionPeanut(CrossSection):
    ...
    def load_data(self):
        ...
        egrid = np.loadtxt(
            join(cspath, "peanut_egrid.grid"), dtype='float') * 1e-3
        print "DEBUG: Egrid loading finished"
        # the first columns denote particle ids for the crosssections
        pid_nonel = np.loadtxt(
            join(cspath, "peanut_IAS_nonel.dat"), usecols=(0, ), dtype='int')
        pids_incl = np.loadtxt(
            join(cspath, "peanut_IAS_incl_i_j.dat"),
            usecols=(0, 1),
            dtype='int')
        ...
        crosssec_nonel_raw = np.loadtxt(
            join(cspath, "peanut_IAS_nonel.dat"), dtype='float') * 1e-27
        crosssec_incl_raw = np.loadtxt(
            join(cspath, "peanut_IAS_incl_i_j.dat"), dtype='float') * 1e-27
        ...
        for key in crosssec_nonel_grid:
            crosssec_nonel[key] = InterpolatedUnivariateSpline(
                egrid, crosssec_nonel_grid[key], k=1, ext='zeros')
        crosssec_incl = {}
        for key in crosssec_incl_grid:
            crosssec_incl[key] = InterpolatedUnivariateSpline(
                egrid, crosssec_incl_grid[key], k=1, ext='zeros')
...
class CrossSectionSophia(CrossSection):
    ...
    def load_data(self):
        ...
        pids = [
            4115, 4116, 4117, 4118, 4119, 4120, 4121, 4122, 2107, 2108, 2109,
            2110, 2111, 2112, 2113, 6224, 6225, 6226, 100, 101, 4214, 4216,
            4218, 4219, 4220, 4221, 4222, 4224, 2206, 2208, 2209, 2210, 2211,
            2212, 2213, 2214, 6323, 6324, 6325, 6326, 201, 4315, 4317, 4318,
            4319, 4320, 4321, 4322, 4324, 2308, 2309, 2310, 2311, 2312, 2313,
            2314, 6424, 6425, 6426, 301, 302, 4416, 4417, 4418, 4419, 4420,
            4421, 4422, 4423, 4424, 2408, 2410, 2411, 2412, 2413, 2414, 6524,
            6525, 6526, 401, 402, 403, 4517, 4518, 4519, 4520, 4521, 4522,
            4523, 4524, 4526, 2509, 2510, 2511, 2512, 2513, 2514, 2515, 6626,
            502, 503, 4616, 4618, 4619, 4620, 4621, 4622, 4623, 4624, 4625,
            4626, 2608, 2609, 2610, 2611, 2612, 2613, 2614, 2615, 6725, 6726,
            601, 602, 603, 604, 4718, 4719, 4720, 4721, 4722, 4723, 4724, 4725,
            4726, 2709, 2710, 2711, 2712, 2713, 2714, 2715, 2716, 6826, 702,
            703, 704, 705, 4816, 4818, 4819, 4820, 4821, 4822, 4823, 4824,
            4825, 4826, 2810, 2811, 2812, 2813, 2814, 2815, 2816, 6926, 802,
            803, 804, 805, 806, 4919, 4920, 4921, 4922, 4923, 4924, 4925, 4926,
            2909, 2910, 2911, 2912, 2913, 2914, 2915, 2916, 2917, 902, 903,
            904, 905, 906, 5018, 5019, 5020, 5021, 5022, 5023, 5024, 5025,
            5026, 3010, 3011, 3012, 3013, 3014, 3015, 3016, 3017, 1003, 1004,
            1005, 1006, 5117, 5119, 5120, 5121, 5122, 5123, 5124, 5125, 5126,
            3111, 3112, 3114, 3115, 3116, 3118, 1103, 1104, 1105, 1106, 5219,
            5220, 5221, 5222, 5223, 5224, 5225, 5226, 3210, 3211, 3212, 3213,
            3214, 3215, 3216, 3217, 3218, 1204, 1205, 1206, 1207, 1208, 5319,
            5320, 5321, 5322, 5323, 5324, 5325, 5326, 3311, 3312, 3313, 3314,
            3315, 3316, 3317, 3318, 1304, 1305, 1306, 1307, 1308, 5420, 5422,
            5423, 5424, 5425, 5426, 3412, 3414, 3415, 3416, 3417, 3418, 3419,
            1404, 1405, 1406, 1407, 1408, 5520, 5521, 5522, 5523, 5524, 5525,
            5526, 3512, 3515, 3516, 3517, 3518, 3519, 1506, 1507, 1508, 1509,
            5620, 5621, 5622, 5623, 5624, 5625, 5626, 3612, 3614, 3615, 3616,
            3617, 3618, 3619, 3620, 1605, 1606, 1607, 1608, 1609, 1610, 5722,
            5723, 5724, 5725, 5726, 3712, 3714, 3716, 3717, 3718, 3719, 3720,
            1705, 1707, 1708, 1709, 1710, 5822, 5823, 5824, 5825, 5826, 3814,
            3815, 3816, 3817, 3818, 3819, 3820, 3822, 1806, 1807, 1808, 1809,
            1810, 5923, 5924, 5925, 5926, 3915, 3916, 3917, 3918, 3919, 3920,
            3921, 3922, 1908, 1909, 1910, 6022, 6024, 6025, 6026, 4014, 4015,
            4016, 4017, 4018, 4019, 4020, 4021, 4022, 2006, 2008, 2009, 2010,
            2011, 2012, 6122, 6123, 6124, 6125, 6126
        ]

        cspath = config['data_dir']
        egrid, cs_proton_grid, cs_neutron_grid = np.loadtxt(
            join(cspath, 'sophia_csec.dat'), delimiter=',', unpack=True)
        ...
        for particle_id in pids:
            ...
            crosssec_nonel[particle_id] = InterpolatedUnivariateSpline(
            ...
            crosssec_incl[particle_id, particle_id -
                          101] = InterpolatedUnivariateSpline(
                              egrid, cgrid, k=1, ext='zeros')
            crosssec_incl[particle_id, 101] = InterpolatedUnivariateSpline(
                egrid, cgrid, k=1, ext='zeros')

            cgrid = neutron_num * cs_neutron_grid
            crosssec_incl[particle_id, particle_id -
                          100] = InterpolatedUnivariateSpline(
                              egrid, cgrid, k=1, ext='zeros')
            crosssec_incl[particle_id, 100] = InterpolatedUnivariateSpline(
                egrid, cgrid, k=1, ext='zeros')
