...
class ContinuousAdiabaticLossRate(object):
    ...
    def __init__(self, prince_run, *args, **kwargs):
        ...
        self.dim_ph = self.e_photon.d
        # The call to setup initializes all variables
        self._init_adiabatic_vec()

    def loss_vector_grid(self, z):
        """Returns adiabatic loss dE/dt at redshift z for egrid"""
        from prince.cosmology import H
        return H(z) * PRINCE_UNITS.cm2sec * self.adiabatic_loss_grid 

    def loss_vector_bins(self, z):
        """Returns adiabatic loss dE/dt at redshift z for ebins"""
        from prince.cosmology import H
        return H(z) * PRINCE_UNITS.cm2sec * self.adiabatic_loss_bins

    def _init_adiabatic_vec(self):
        """Prepare vector for scaling with units, charge and mass."""

        adiabatic_loss_grid = np.zeros(self.dim_cr)
        egrid = self.e_cosmicray.grid
        for spec in self.spec_man.species_refs:
            adiabatic_loss_grid[spec.lidx():
                                  spec.uidx()] = egrid
        self.adiabatic_loss_grid = adiabatic_loss_grid

        adiabatic_loss_bins = np.zeros(self.dim_cr + 1)
        ebins = self.e_cosmicray.bins
        for spec in self.spec_man.species_refs:
            adiabatic_loss_bins[spec.lbin():
                                  spec.ubin()] = ebins
        self.adiabatic_loss_bins = adiabatic_loss_bins
...
class ContinuousPairProductionLossRate(object):
    ...
    def loss_vector_grid(self, z):
        """Returns adiabatic loss dE/dt at redshift z for (egrid, ebins)"""
        self._update_rates(z)
        return self.pprod_loss_grid

    def loss_vector_bins(self, z):
        """Returns adiabatic loss dE/dt at redshift z for (egrid, ebins)"""
        self._update_rates(z)
        return self.pprod_loss_bins
    ...
    def _setup(self):
        ...
        self._photon_grid = np.zeros_like(self.photon_grid)
        self._ph_grid_zcache = None 

        # Gamma factor of the cosmic ray
        gamma = self.e_cosmicray.bins / PRINCE_UNITS.m_proton
        # Grid of photon energies for interpolation
        self.photon_bins = np.outer(1 / gamma,
                                    self.xi) * PRINCE_UNITS.m_electron / 2.
        self.pb_desort = self.photon_bins.reshape(-1).argsort()
        self.pb_sorted = self.photon_bins.reshape(-1)[self.pb_desort]
        self._photon_bins = np.zeros_like(self.photon_bins)
        self._ph_bins_zcache = None

        # Rate vector (dim_states x 1) containing all rates
        self.pprod_loss_grid = np.zeros(self.dim_cr)
        self.pprod_loss_bins = np.zeros(self.dim_cr + 1)
    ...
    def _update_rates(self, z):
        ...
        np.multiply(
            self.scale_grid,
            np.tile(
                trapz(self.get_photon_grid(z) * self.phi_xi2, self.xi, axis=1),
                self.spec_man.nspec),
            out=self.pprod_loss_grid)

        np.multiply(
            self.scale_bins,
            np.tile(
                trapz(self.get_photon_bins(z) * self.phi_xi2, self.xi, axis=1),
                self.spec_man.nspec),
            out=self.pprod_loss_bins)

    def get_photon_grid(self, z):
        if self._ph_grid_zcache == z:
            return self.photon_grid
        else:
            self._photon_grid.reshape(-1)[
                self.pg_desort] = self.photon_field.get_photon_density(
                    self.pg_sorted, z)
            self._ph_grid_zcache = z
            return self.photon_grid

    def get_photon_bins(self, z):
        if self._ph_bins_zcache == z:
            return self.photon_bins
        else:
            self._photon_bins.reshape(-1)[
                self.pb_desort] = self.photon_field.get_photon_density(
                    self.pb_sorted, z)
            self._ph_bins_zcache = z
            return self.photon_bins
    ...
    def _init_scale_vec(self):
        ...
        scale_grid = np.zeros(self.prince_run.dim_states)
        for spec in self.spec_man.species_refs:
            if not spec.is_nucleus:
                continue
            scale_grid[spec.lidx():spec.uidx()] = units * abs(
                spec.charge) * spec.Z**2 / float(spec.A) * np.ones(
                    self.dim_cr, dtype='double')
        self.scale_grid = scale_grid

        scale_bins = np.zeros(self.prince_run.dim_bins)
        for spec in self.spec_man.species_refs:
            if not spec.is_nucleus:
                continue
            scale_bins[spec.lidx():spec.uidx()] = units * abs(
                spec.charge) * spec.Z**2 / float(spec.A) * np.ones(
                    self.dim_cr, dtype='double')
